### LANGCHAIN + CHROMA + Ollama (chromadb + embedding) + GROQ ###
#https://python.langchain.com/api_reference/groq/chat_models/langchain_groq.chat_models.ChatGroq.html#langchain_groq.chat_models.ChatGroq
...
def get_response_from_website_with_chunks(
    ...
    try:
        ...
        llm = ChatGroq(
        model_name=llm_model_name,  # Or another available Groq model
        groq_api_key=groq_api_key
            )
        try:
            response = llm.invoke(f"""Answer the question according to the job offer context given. Reply in English and try to guess the most out of the Project, including how to set it up if its explicitely mentioned. Justify your answer:
                    Question: {question}.
                    Context: {context}""")
            print(response)
